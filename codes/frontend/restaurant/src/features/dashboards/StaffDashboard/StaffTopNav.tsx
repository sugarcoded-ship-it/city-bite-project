...
export function StaffTopNav() {
  ...
  const links = [
    ...
    { label: 'Day-Off', icon: CalendarPlus, path: '/staff/dayoff', badge: 0 },
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-[#0B1F4D] shadow-lg">
        <div className="max-w-7xl mx-auto px-5 h-16 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 flex-shrink-0">
            <button onClick={() => navigate('/staff/dashboard')} className="flex items-center gap-2 focus:outline-none">
              <img src="/citybite-logo-navy-alt.png" alt="City Bite Logo" className="h-9 w-auto object-contain" />
              <span className="text-blue-300 text-[10px] font-bold tracking-widest uppercase hidden sm:block">
                Staff Portal
              </span>
            </button>
            {isOwner && (
              <button onClick={() => navigate('/owner/dashboard')} className="flex items-center gap-1.5 bg-white/10 hover:bg-white/15 border border-white/20 px-3 py-1.5 rounded-full text-white text-xs font-bold transition-colors">
                <ArrowLeft size={13} />
          ...
          <nav className="flex items-center gap-1">
            {links.map(({ label, icon: Icon, path, badge }) => {
              ...
              return (
                <button
                  ...
                  className={`${styles.navButton} ${active ? styles.navButtonActive : styles.navButtonInactive}`}
                >
            ...
            <button onClick={handleLogout} title="Logout" className="w-9 h-9 ml-2 rounded-xl bg-red-500/10 flex items-center justify-center text-red-400 hover:bg-red-500/20 transition-colors flex-shrink-0">
              <LogOut size={17} />
            </button>
          </nav>
        </div>
